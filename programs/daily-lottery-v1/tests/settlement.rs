...
fn begin_reveal_now(
    ...
    let mut clock = ctx.get_clock();
    clock.unix_timestamp = lot.upload_start_unix;
    ctx.set_clock(&clock);
}
...
fn single_participant_refund_can_settle_before_upload_deadline() {
    ...
    let lot = load_lottery(&mut ctx, lottery_pda);
    assert!(!lot.uploads_complete); // Clock progression needs no timing mutation transaction.
...
#[test]
fn absolute_timeout_refunds_partial_selection_but_never_vested_winners() {
    for vest in [false, true] {
        let program_id = Pubkey::new_unique();
        let authority = Keypair::new();
        let a = Keypair::new();
        let b = Keypair::new();
        let mut ctx = TestContext::new(program_id, &[&authority, &a, &b]);
        let (config, lottery, vault, _) = setup_lottery(&mut ctx, program_id, &authority);
        let pa = buy_tickets(
            &mut ctx,
            program_id,
            config,
        ...
        begin_reveal_now(&mut ctx, program_id, config, lottery, &authority, 0, 0);
        let tally = Pubkey::find_program_address(&[b"vote_tally", lottery.as_ref()], &program_id).0;
        for (buyer, participant, phrase) in [(&a, pa, b"timeout-a"), (&b, pb, b"timeout-b")] {
            let ix = SdkIx {
                program_id,
                accounts: vec![
                ...
                data: borsh::to_vec(&Instruction::AttestReveal {
                    voted_number_of_winners: 1,
                    reveal_plaintext: phrase.to_vec(),
                })
                .unwrap(),
            };
            send_tx(&mut ctx, vec![ix], &[buyer]).unwrap();
        ...
        if vest {
            ...
            // Finish aggregation, leaving selection incomplete and no vested entitlement.
            finalize_winners_chunk(
                &mut ctx,
                program_id,
                config,
            ...
            .unwrap();
            assert_eq!(load_lottery(&mut ctx, lottery).winners_count, 0);
        }
        let before = load_lottery(&mut ctx, lottery);
        let mut clock = ctx.get_clock();
        clock.unix_timestamp = before.settlement_deadline_unix;
        ctx.set_clock(&clock);
        if !vest {
            assert!(finalize_winners_chunk(
                &mut ctx,
        ...
        let result = send_tx(&mut ctx, vec![refund], &[]);
        if vest {
            assert!(result.is_err());
            let page = winner_page_pda(&program_id, &lottery, 0);
            let winner = load_winner(&mut ctx, page, 0);
            let balance = ctx.get_account(winner).unwrap().lamports;
            let payout = SdkIx {
                program_id,
                accounts: vec![
                    AccountMeta::new_readonly(config, false),
                ...
                data: borsh::to_vec(&Instruction::SettlePayoutBatch {
                    lottery_id: before.id,
                    batch_index: 0,
                    winners: vec![WinnerProof {
                        index: 0,
                        recipient: winner,
                        amount: before.total_payout,
                        merkle_proof: vec![],
                    }],
                })
                .unwrap(),
            };
            send_tx(&mut ctx, vec![payout], &[]).unwrap();
            assert_eq!(
                ctx.get_account(winner).unwrap().lamports,
                balance + before.total_payout
            );
        } else {
            result.unwrap();
            for (buyer, participant) in [(&a, pa), (&b, pb)] {
                let balance = ctx.get_account(buyer.pubkey()).unwrap().lamports;
                claim_refund(
                    &mut ctx,
                    program_id,
                    config,
                ...
                .unwrap();
                assert_eq!(
                    ctx.get_account(buyer.pubkey()).unwrap().lamports,
                    balance + 1_000_000
                );
            }
        }
