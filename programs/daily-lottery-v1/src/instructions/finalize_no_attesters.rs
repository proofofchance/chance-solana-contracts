...
pub fn process(program_id: &Pubkey, accounts: &[AccountInfo]) -> ProgramResult {
    ...
    require_key_match(authority_ai, &lottery.authority)?;
    ...
    let single_participant_ready =
        lottery.participants_count == 1 && clock.unix_timestamp >= lottery.buy_deadline_unix;
    ...
    let timed_out = lottery.settlement_expired(clock.unix_timestamp);
    if !(no_attestations || remediation_expired || timed_out) {
        return Err(Error::InvalidInstruction.into());
    }
    ...
    let refunds_event = LotteryEvent::RefundsIssued {
        ...
        reason: if lottery.participants_count == 1 {
            ...
            "Settlement deadline expired before winner vesting".to_string()
