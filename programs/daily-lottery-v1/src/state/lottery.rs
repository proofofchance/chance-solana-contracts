...
pub const LOTTERY_ACCOUNT_VERSION: u16 = 2;
...
pub struct Lottery {
    ...
    /// Absolute last instant for vesting winners; timeout opens refunds.
...
impl Default for Lottery {
    fn default() -> Self {
        Self {
            ...
            account_version: 0,
            max_winners_cap: crate::state::sizes::MAX_WINNERS as u32,
            settlement_deadline_unix: 0,
            reserved: [0; 114],
        }
    }
}
...
impl Lottery {
    ...
    pub fn new(
        ...
        Self {
            ...
            max_winners_cap: crate::state::sizes::MAX_WINNERS as u32,
            settlement_deadline_unix: upload_deadline_unix
                .saturating_add(2 * crate::state::sizes::DEFAULT_REMEDIATION_WINDOW_SECS),
            reserved: [0; 114],
        }
    }
    ...
    fn calculate_windows(created_at_unix: i64) -> (i64, i64, i64, i64) {
        ...
        let buy_deadline = created_at_unix.saturating_add(24 * 60 * 60); // 24 hours buy window
        let upload_start = created_at_unix.saturating_add(24 * 60 * 60); // default: start after buy ends
        let upload_deadline = created_at_unix.saturating_add(48 * 60 * 60); // default: end 24h after upload begins
        (buy_start, buy_deadline, upload_start, upload_deadline)
    }

    /// Called only during creation, before the account is written or funded.
    pub fn freeze_config(
        ...
        if self.created_at_unix <= 0
            ...
            || config.upload_window_secs == 0
            || !crate::state::Config::validate_max_winners_cap(config.max_winners_cap)
        {
            return Err(Error::InvalidInstruction);
        }
        let buy_end = self
            .created_at_unix
            .checked_add(i64::from(config.buy_window_secs))
            .ok_or(Error::MathOverflow)?;
        let upload_end = buy_end
            .checked_add(i64::from(config.upload_window_secs))
            .ok_or(Error::MathOverflow)?;
        let settlement_end = upload_end
            .checked_add(2 * crate::state::sizes::DEFAULT_REMEDIATION_WINDOW_SECS)
            .ok_or(Error::MathOverflow)?;
        self.buy_start_unix = self.created_at_unix;
        self.buy_deadline_unix = buy_end;
        self.upload_start_unix = buy_end;
        self.upload_deadline_unix = upload_end;
        self.settlement_deadline_unix = settlement_end;
        self.max_winners_cap = config.max_winners_cap;
        Ok(())
    }

    pub fn effective_max_winners(&self) -> u64 {
        u64::from(self.max_winners_cap)
            .min(crate::state::sizes::MAX_WINNERS as u64)
            .min(self.participants_count.saturating_sub(1).max(1))
    }

    pub fn settlement_expired(&self, now: i64) -> bool {
        self.settlement_deadline_unix > 0 && now >= self.settlement_deadline_unix
    }

    pub fn has_vested_winners(&self) -> bool {
        self.winners_count != 0 || self.total_payout != 0 || self.winners_merkle_root != [0; 32]
    }
    ...
    pub fn begin_remediation(&mut self, current_time: i64) {
        if self.remediation_start_unix == 0 {
            ...
            self.remediation_deadline_unix = self
                .upload_deadline_unix
                .saturating_add(crate::state::sizes::DEFAULT_REMEDIATION_WINDOW_SECS);
        }
    }
    ...
    pub fn initialize_settlement(
        ...
        if self.settled || self.settlement_complete || self.winners_count != 0 {
...
mod tests {
    ...
    fn test_lottery_creation() {
        ...
        assert_eq!(lottery.reserved, [0; 114]);
        assert!(lottery.is_active());
        assert!(!lottery.settled);
    }

    #[test]
    fn frozen_schedule_has_checked_deadlines_and_preserves_allocation() {
        ...
        let config = crate::state::Config {
            buy_window_secs: 60,
            upload_window_secs: 120,
            max_winners_cap: 3,
            ..crate::state::Config::default()
        };
        lottery.freeze_config(&config).unwrap();
        lottery.participants_count = 10;
        assert_eq!(lottery.buy_deadline_unix, 1_060);
        assert_eq!(lottery.upload_deadline_unix, 1_180);
        assert_eq!(lottery.effective_max_winners(), 3);
        assert_eq!(
            lottery.settlement_deadline_unix,
            1_180 + 2 * crate::state::sizes::DEFAULT_REMEDIATION_WINDOW_SECS
        );
        assert!(!lottery.settlement_expired(lottery.settlement_deadline_unix - 1));
        assert!(lottery.settlement_expired(lottery.settlement_deadline_unix));
        assert_eq!(
            8 + borsh::to_vec(&lottery).unwrap().len(),
            crate::state::sizes::LOTTERY_SIZE
        );
        lottery.begin_remediation(lottery.upload_deadline_unix + 10);
        let deadline = lottery.remediation_deadline_unix;
        lottery.begin_remediation(deadline + 100);
        assert_eq!(deadline, lottery.remediation_deadline_unix);
        assert_eq!(
            deadline,
            lottery.upload_deadline_unix + crate::state::sizes::DEFAULT_REMEDIATION_WINDOW_SECS
        );
    }

    #[test]
    fn overflowing_creation_rejects_without_partial_rule_changes() {
        let mut lottery = Lottery::new(
            1,
            Pubkey::new_unique(),
            Pubkey::new_unique(),
        ...
        let before = borsh::to_vec(&lottery).unwrap();
        assert!(lottery.freeze_config(&config).is_err());
        assert_eq!(before, borsh::to_vec(&lottery).unwrap());
    }

    #[test]
    fn vested_entitlements_cannot_be_reinitialized() {
        let mut lottery = Lottery::new(
            1,
            Pubkey::new_unique(),
            Pubkey::new_unique(),
        ...
        lottery.initialize_settlement([1; 32], 1, 100).unwrap();
        lottery.mark_winner_paid(0).unwrap();
        assert!(lottery.initialize_settlement([2; 32], 1, 100).is_err());
        assert!(lottery.is_winner_paid(0));
        assert_eq!(lottery.winners_merkle_root, [1; 32]);
    }
    ...
    fn test_stateless_phase_computation() {
        ...
        assert_eq!(lottery.phase(3000), "settlement"); // exclusive deadline
